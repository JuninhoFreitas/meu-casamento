export const casamento = {
  noivos: {
    noivo: 'João',
    noiva: 'Gabrielle',
    nomeCompleto: 'Gabrielle & João',
    iniciais: 'G&J',
  },
  data: {
    data: '07/03/2026',
    dataISO: '2026-03-07',
    horario: {
      cerimonia: '15:00',
      observacao:
        'Mencionado também 9h00 no Hero, mas 15:00 é o horário principal da cerimônia',
    },
  },
  local: {
    cerimonia: {
      nome: 'Igreja Bíblica Unidos no Senhor',
      cidade: 'Novo Hamburgo',
      estado: 'RS',
      pais: 'Brasil',
      enderecoCompleto:
        'R Machado de Assis, 620 - Ideal, Novo Hamburgo - RS, Brasil',
    },
  },
  hospedagem: {
    descricao: 'Vamos disponibilizar um guia para hospedagens',
    preco: 'R$?/noite',
    tags: ['Acomodação', 'Perto do local', 'Reservas até 10/09'],
    prazoReserva: '10/09',
  },
  presentes: {
    descricao: 'Sugestões de presentes para compartilhar nossa felicidade',
    tipoPreferido: 'Monetário preferido',
    tags: ['Presentes', 'PIX disponível', 'Contato direto'],
    observacao: 'PIX disponível para contribuições',
  },
  confirmacaoPresenca: {
    descricao:
      'Ajude-nos a organizar melhor o evento confirmando sua presença',
    tipo: 'RSVP',
    tags: ['Presença', 'Organização', 'Contato conosco'],
    email: 'brizollajr@gmail.com',
  },
  citacaoBiblica: {
    texto: 'Dêem graças ao Senhor, porque Ele é bom',
    referencia: 'Salmos 136.1a',
    textoCompleto: '"Dêem graças ao Senhor, porque ele é bom!" Salmos 136.1a',
  },
  historia: {
    titulo: 'Nossa história de amor',
    eventos: [
      {
        titulo: 'Dia que nos conhecemos',
        data: '16/09/2024',
        dataISO: '2024-09-16',
        noivo: {
          depoimento:
            'Dia 16 de setembro de 2024, nos encontramos pela primeira vez. Naquele dia, eu retornava de uma missão no Amazonas onde fomos ensinar sobre aconselhamento bíblico para os Ribeirinhos.',
        },
        noiva: {
          depoimento:
            'Eu estava vindo do Maranhão, onde morava após concluir a faculdade de medicina. Foi na Conferência de Aconselhamento Bíblico em Atibaia que nossos caminhos se cruzaram.',
        },
        local: 'Conferência de Aconselhamento Bíblico em Atibaia',
      },
      {
        titulo: 'Começo da Amizade',
        data: '19/09/2024',
        dataISO: '2024-09-19',
        noivo: {
          depoimento:
            'Começamos a conversar mais frequentemente sobre ministério e vida. Logo percebi que tínhamos muito em comum em nossa visão de vida e serviço.',
        },
        noiva: {
          depoimento:
            'Nossa amizade cresceu naturalmente, compartilhando experiências de ministério e sonhos para o futuro.',
        },
      },
      {
        titulo: 'Começo do Namoro',
        data: '21/10/2024',
        dataISO: '2024-10-21',
        noivo: {
          depoimento:
            'Após meses de oração e aconselhamento, decidimos iniciar um relacionamento com propósito de casamento.',
        },
        noiva: {
          depoimento:
            'Foi um momento especial quando decidimos dar esse passo juntos, confiando na direção de Deus.',
        },
      },
      {
        titulo: 'Pedido de Casamento',
        data: '18/05/2025',
        dataISO: '2025-05-18',
        noivo: {
          depoimento:
            'Planejei cada detalhe do pedido, querendo tornar esse momento inesquecível para nós dois.',
        },
        noiva: {
          depoimento:
            'Foi uma surpresa maravilhosa! Mal posso esperar para começar nossa vida juntos.',
        },
      },
      {
        titulo: 'Casamento',
        data: '07/03/2026',
        dataISO: '2026-03-07',
        noivo: {
          depoimento: '...',
        },
        noiva: {
          depoimento: '...',
        },
      },
    ],
  },
  roadmap: {
    titulo: 'Roadmap da Nossa Celebração',
    subtitulo: 'Sua missão, caso decida aceitar, é celebrar com a gente!',
    etapas: [
      {
        numero: '01',
        titulo: 'A Convocação',
        descricao:
          'Mergulhamos fundo na sua agenda para garantir sua presença! Nesta primeira fase, pedimos que você confirme sua vinda, nos informe se precisará de ajuda com hospedagem e se utilizará o suporte dos Padrinhos para a entrega de presentes. Suas respostas são a base do nosso planejamento!',
      },
      {
        numero: '02',
        titulo: 'Contagem Regressiva',
        descricao:
          'Com as informações iniciais, entramos em modo de preparação. Conforme o grande dia se aproxima, enviaremos lembretes e pediremos as confirmações finais para que tudo esteja perfeito. Fique de olho nas nossas comunicações!',
      },
      {
        numero: '03',
        titulo: 'O Grande Dia',
        descricao:
          'O desenvolvimento do nosso sonho se torna realidade! Aqui, sua única tarefa é vir para o casamento com o coração aberto, o melhor sorriso no rosto e muita energia para celebrar o nosso amor.',
      },
      {
        numero: '04',
        titulo: 'A Comemoração',
        descricao:
          'Lançamento oficial da nossa nova vida! Após a cerimônia, queremos todos juntos para um almoço delicioso, onde poderemos conversar, rir e criar as primeiras memórias de um dia inesquecível. A sua presença é o nosso maior presente.',
      },
      {
        numero: '05',
        titulo: 'Boas Lembranças',
        descricao:
          'O evento termina, mas a celebração continua em nossos corações. Esta fase é eterna, onde guardaremos com carinho cada abraço, cada foto e a alegria de ter tido você ao nosso lado. Muito obrigado por fazer parte da nossa história!',
      },
    ],
  },
  contato: {
    email: 'brizollajr@gmail.com',
    opcoes: [
      {
        tipo: 'Confirmação de Presença',
        assunto: 'Confirmação de Presença - Casamento Gabrielle & João',
        email: 'brizollajr@gmail.com',
      },
      {
        tipo: 'Lista de Presentes',
        assunto: 'Lista de Presentes - Casamento Gabrielle & João',
        email: 'brizollajr@gmail.com',
      },
      {
        tipo: 'Mural de Recados',
        assunto: 'Mural de Recados - Casamento Gabrielle & João',
        email: 'brizollajr@gmail.com',
      },
    ],
    mensagem:
      'Tem dúvidas sobre o casamento? Precisa confirmar presença? Entre em contato conosco!',
  },
  informacoesAdicionais: {
    observacoes: [
      'Novas informações estão chegando, lentamente mas certamente 😅',
      'Após a cerimônia haverá um almoço delicioso',
      'É possível utilizar o suporte dos Padrinhos para entrega de presentes',
    ],
  },
  secoes: {
    hero: {
      titulo: 'Gabrielle & João',
      subtitulo:
        '7 de Março de 2026 — 9h00 Faltam {daysLeft} dias para o nosso grande dia!',
      observacao:
        'Nota: Há uma discrepância entre o horário mencionado no Hero (9h00) e no Works (15:00). O horário de 15:00 parece ser o correto para a cerimônia.',
    },
    infos: {
      titulo: `Informações
do
casamento`,
      subtitulo: 'Tudo que você precisa saber sobre o nosso grande dia',
    },
    ourStorySectionInfo: {
      titulo: 'Nossa história de amor',
    },
    roadmapSectionInfo: {
      titulo: 'Roadmap da Nossa Celebração',
      subtitulo: 'Sua missão, caso decida aceitar, é celebrar com a gente! =)',
    },
    connectSectionInfo: {
      titulo: 'Entre em contato conosco',
      subtitulo:
        'Tem dúvidas sobre o casamento? Precisa confirmar presença? Entre em contato conosco!',
    },
  },
  metadata: {
    site: {
      copyright:
        '© 2025 Gabrielle e João. Celebrando nosso amor com muito carinho e gratidão.',
      navegacao: [
        {
          id: 'works',
          label: 'Informações',
        },
        {
          id: 'story',
          label: 'Nossa História',
        },
        {
          id: 'process',
          label: 'Roadmap',
        },
        {
          id: 'connect',
          label: 'Contato',
        },
      ],
    },
  },
}



